...
public class DeathsInTheNight {
    ...
    @Getter @Setter private static int hero_max_uses = WerewolfMain.config.getInt("role_info.hero.uses");
    @Getter private static int protector_max_uses = WerewolfMain.config.getInt("role_info.protector.uses");

    private static final List<Role> offensiveRoles = Arrays.asList(Role.ALPHA_WOLF, Role.BETA_WOLF, Role.OMEGA_WOLF, Role.MURDERER);

    @Getter @Setter private static boolean alphaIsAlive = true;
    @Getter @Setter private static boolean betaIsAlive = true;
